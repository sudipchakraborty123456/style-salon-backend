                ...
                let request = https.request(options,(responseFromPaytmServer) => {
                    ...
                    responseFromPaytmServer.on('end',() => {
                        ...
                        res.sendFile(__dirname+'/success.html')
                         // res.status(400).json({message : "payment succesfull"})
                           // console.log(orderId)
                            order.updateOne({orderId:orderId },{$set :{confirmBooking:'confirm'}}).then((result) => {
                                console.log("db updated");

                            }).catch((err)=>{
                                console.log(err);
                            })
                      //  db.Orders.update({orderId: params['ORDER_ID']},{$set:{confirmBooking:'confirm'}});
                        //  res.sendFile(__dirname+'/success.html')
